function Chart(){
    ...
    me.drawAxis = function(){
        ...
        me.svg.append(rect);
        // 차트 박스 M L 라인 생성 M = 라인 을 이동 시킴 L = 라인을 그려줌 
    ...
    me.setData = function(data){
        ...
        for(let i=0; i < dataCount; i++){
	        ...
	        textValue.setAttribute('x', barLeft + barWidth / 2);
	        textValue.setAttribute('y', barTop - 5); // 텍스트를 막대 위에 배치하기 위해 위치 조정
	        textValue.innerHTML = item.y.toFixed(2); // 소수점 두 자리까지 값 표시
	        textValue.setAttribute('dominant-baseline', 'baseline');
